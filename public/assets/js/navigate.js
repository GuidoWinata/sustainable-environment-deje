...
$(document).on('click', 'a:not([navigate]):not([navigate-api]):not([navigate-api-confirm]):not(.sidebar-link):not(.note-dropdown-item)', function (e) {
    e.preventDefault();
...
$(document).on('submit', 'form[navigate-form]', function (e) {
    ...
    if (navigator.onLine) {
        ...
        if (hasFile) {
            formData = new FormData(form[0]); // Gunakan FormData jika ada file
        } else {
            formData = form.serialize(); // Gunakan serialize untuk data biasa
        }
